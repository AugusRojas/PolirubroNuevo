...
namespace POLIRUBRO.capaPresentacion
{
    public partial class Proveedores : Form
    {
        ...
        private void txtCUIT_Validating(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            if (!System.Text.RegularExpressions.Regex.IsMatch(cuit, patron))
            {
                MessageBox.Show("El CUIT debe tener el formato XX-XXXXXXXX-X.", "Formato Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                e.Cancel = true;
            }
        }
        ...
        private void txtTelefono_Validating(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            string patron = @"^\(\d{3}\) \d{3}-\d{4}$|^\d{3}-\d{7}$";
            if (!System.Text.RegularExpressions.Regex.IsMatch(telefono, patron))
            {
                MessageBox.Show("El teléfono debe tener el formato (XXX) XXX-XXXX o XXX-XXXXXXX.", "Formato Incorrecto", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                e.Cancel = true;
            }
        }
